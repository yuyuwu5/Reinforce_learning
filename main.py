...
def parse():
    ...
    parser.add_argument('--train_duel', action='store_true', help='whether train DQN')
...
def run(args):
    ...
    if args.train_ddqn:
        env_name = args.env_name or 'MsPacmanNoFrameskip-v0'
        env = Environment(env_name, args, atari_wrapper=True)
        from agent_dir.agent_ddqn import AgentDQN
        agent = AgentDQN(env, args)
        agent.train()
    ...
    if args.train_duel:
        env_name = args.env_name or 'MsPacmanNoFrameskip-v0'
        env = Environment(env_name, args, atari_wrapper=True)
        from agent_dir.agent_duel import AgentDQN
        agent = AgentDQN(env, args)
        agent.train()
