...
class AgentDQN(Agent):
    ...
    def update(self):
        ...
        _,bestAct = self.online_net(batch_next_state).max(1, keepdim=True)
        
        Q_next = self.target_net(batch_next_state).detach().gather(1, bestAct).squeeze(1)
		#print(Q_next)
    ...
    def train(self):
        ...
        while episodes_done_num < 7000:
            ...
            if episodes_done_num % self.display_freq == 0:
                if episodes_done_num % 50 == 0:
                    plt_reward.append(total_reward_p/50)
                    plt_epi.append(episodes_done_num)
                    plt.plot(plt_epi, plt_reward)
                    plt.savefig('ddqn.png')
                    total_reward_p = 0
                    with open('reward_ddqn', 'wb') as f:
                        pickle.dump(plt_reward, f)
                    with open('epi_ddqn', 'wb') as f:
